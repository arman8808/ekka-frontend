...
export default function DecodeComponent() {
      ...
      <div className="w-full lg:w-1/2 relative order-2 lg:order-2 h-full">
        <div className="relative w-full h-[300px] sm:h-[400px] lg:h-[521px] overflow-hidden">
          ...
          <video
            ...
            src={src}
            poster={poster}
            className={`w-full h-full object-cover cursor-pointer transition-opacity duration-300 ${showThumbnail ? 'opacity-0' : 'opacity-100'
              }`}
            onClick={handlePlayPause}
            onTimeUpdate={handleTimeUpdate}
            onLoadedMetadata={handleLoadedMetadata}
  ...
  return (
    ...
    >
      ...
      {/* Right Content Section */}
  <motion.div
  className="relative w-full lg:w-1/2 bg-[#6E2D79] flex flex-col items-center justify-center p-6 sm:p-8 md:p-10 lg:p-12 xl:p-16 text-center order-1 lg:order-2 h-auto min-h-[400px] sm:min-h-[450px] md:h-[521px]"
  variants={containerVariants}
>
  <motion.h2
    className="text-[32px] sm:text-[36px] md:text-[42px] lg:text-[64px] xl:text-[64px] font-medium text-gray-50 font-poppins mb-6 sm:mb-6 lg:mb-8 leading-tight"
    variants={fadeUp}
  >
    DECODE
  </motion.h2>
  ...
  <motion.p
    className="text-[16px] sm:text-[16px] md:text-[18px] lg:text-[18px] text-[#F6F6F6] font-poppins max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg mb-6 sm:mb-6 lg:mb-8 leading-relaxed"
    variants={fadeUp}
  >
    Take charge of the unconscious programmings playing out as fate.
  </motion.p>
