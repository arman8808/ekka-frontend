...
const FamilyConsultationTable = () => {
  ...
  // Fetch consultation data with server-side pagination
  const fetchConsultations = async (page = 1) => {
    setLoading(true);
    setError(null);

    try {
      const url = new URL(`${import.meta.env.VITE_API_BASE_URL}familyConsitalation`);
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex items-center space-x-2 text-gray-600">
              ...
              <span className="text-sm">
                Showing {consultations.length} of {pagination.totalRegistrations} records
              </span>
            </div>
          </div>
